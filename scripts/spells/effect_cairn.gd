## Sortilège "Cairn" : a chaque manche gagnée, compte les Rocks encore sur la
...
## Rocks avant que ce sortilège ne compte si la manche est passee par le Dernier
## Souffle.
...
func apply(event: Dictionary, run_manager: RunManager) -> void:
	...
	if grid_manager == null:
		return
	var total: int = (run_manager.get_run_spell_state(STATE_KEY, 0) as int) + grid_manager.count_rocks()
