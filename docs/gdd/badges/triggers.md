...
Les moments où les Badges s'activent. 8 triggers au total — chaque Badge en a un.
...
Tous les 8 ont au moins un Badge depuis la session 17. `on_round_start` reste le plus chargé (la majorité des Badges à condition statique posée une fois par manche). `on_last_breath` reste câblé mais sans Badge qui le consomme.
