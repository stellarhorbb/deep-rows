...
- **Pas de doublons** de Sortilèges déjà possédées
...
- **File d'apparition pour Sortilèges et Partitions** (session 23, voir `ShopManager._draw_spell_queued`/`_next_sheet_in_queue`) — inspiré de Balatro : contrairement au tirage indépendant ci-dessus, un item vu-et-refusé ne peut pas ressortir avant d'avoir fait le tour complet des autres du même pool (palier de rareté pour les Sortilèges, pool générique/légendaire séparé pour les Partitions). File mélangée une fois par run, remélangée automatiquement une fois épuisée, réinitialisée à chaque nouvelle run (`ShopManager.reset_run`).
- **Taux fixe par palier pour les Sortilèges** (session 23, voir `GameRules.SPELL_RARITY_RATES`) — remplace `RARITY_WEIGHTS` pour les Sortilèges uniquement. COMMON=50%, UNCOMMON=30%, RARE=13%, EPIC=6%, LEGENDARY=1%, indépendant du nombre de Sortilèges dans chaque palier (contrairement au système par poids toujours utilisé par Spéciaux/Dés à coudre) — ajouter un Sortilège à un palier ne rend plus ce palier plus fréquent, juste plus varié. Un palier sans aucun Sortilège disponible (tous équipés) est retiré du tirage et les autres se renormalisent automatiquement, même principe que le "resample" de Balatro. Voir [Rareté des Sortilèges](../sortileges/rarete.md).
...
- [Packs](packs.md)
- [Reroll](reroll.md)
- [Rareté des Sortilèges](../sortileges/rarete.md)
