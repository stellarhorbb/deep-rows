...
**19 Partitions actives** dans le proto (session 12 : la valeur ne résout plus de patterns sur les formes famille, seuls famille et rock restent actifs — voir [Axes de règles](axes-de-regles.md). Session 13 : 4 nouvelles formes — Plus, Cross, Ring, T. Session 14 : Rainbow sur l'axe famille + retour de la valeur sous vocabulaire casino confiné aux lignes, voir [Formes](formes.md) et [Axes de règles](axes-de-regles.md). Session 16 : retrait de l'axe directionnel, chaque Partition — lignes comprises — a désormais un multiplicateur fixe unique, calibré par tier de difficulté réelle plutôt que par géométrie brute. Session 19 : Minima, Maxima, Prime — trois nouvelles Partitions casino jouables sur tout le run, pas seulement en fin de run comme le tiroir rare/signature, voir [Axes de règles](axes-de-regles.md#genèse--pourquoi-ces-trois-là-session-19)).
...
| Line 4          | Ligne             | Famille                                             | 4          | any       | ×1.5  | 6    |
...
| Suite           | Ligne             | Valeurs consécutives (any sens)                     | 4          | any       | ×2    | 14   |
...
| Amorce | Brelan, Minima, Line 4 | ×1.5 |
| Facile | Square 4, Small T, Diamond Rainbow, Prime, Suite | ×2 |
| Medium | Line 5, Carré (poker), Fibonacci, Diamond, Number Square (dormant) | ×2.5 |
| Difficile | Plus, Maxima | ×3 |
| Très dur | Cross | ×4 |
| Extrême | Ring | ×5 |
| Hors échelle | Diamond Rock, Rock Bookends, 777, 9999 | cas à part, voir ci-dessous |
...
- **Minima/Maxima/Prime (session 19)** : Minima reste au tier Amorce comme Brelan (filet d'entrée facile à satisfaire avec le deck de départ) ; Maxima grimpe à Difficile comme Plus (nécessite plusieurs Fusions pour atteindre des valeurs > 7, jamais tirées telles quelles) ; Prime se cale au tier Facile, en dessous de Fibonacci/Carré — lire des nombres premiers demande le même effort de lecture que le reste de l'axe casino, mais 2/3/5/7 restent des valeurs courantes du deck de base
- **Line 4 et Suite retunées (session 23)** — playtest d'une run complète en spam mono-Partition (Line 3 verticale) : aucune pression de grille, aucun recours à la Fusion, multi qui explose (voir [Questions ouvertes](../meta/questions-ouvertes.md)). L'intention de session 23 était de retirer Line 3 du catalogue plutôt que de la restreindre par direction — imposer une direction va contre la lecture naturelle de la grille par le joueur (scan toutes directions confondues), jugé mauvais pour la cognition/UX. **Correction (constaté session 30)** : Line 3 n'a en fait jamais été retirée du code — `line_family_3.tres` reste dans le pool shop (`ShopManager.SHEET_PATHS`) et sert même de socle d'onboarding imposé (`fixed_sheet_a`) sur les 4 starters day-one, pour donner un point d'entrée lisible ("aligne des couleurs") avant de vendre et viser plus fort en cours de run. Line 4 reste la Partition famille-ligne la plus accessible *parmi les choix libres du shop*, son multiplicateur descend ×2→×1.5 pour hériter de la niche "faible mais pas cher" de l'ancienne Line 3, tout en gardant la contrainte min 4 (un cran de plus qu'avant, donc plus exposée à la pression de grille). Suite descend ×2.5→×2 (rejoint le tier Facile) pour rétablir un écart facile/dur dans Le Prévoyant une fois réassociée à Diamond (×2.5) — voir [Structure du run](../progression/structure-run.md#choix-de-départ) pour les 4 packs day-one retravaillés en conséquence. **Décision (session 30)** : Line 3 reste volontairement achetable au shop malgré la redondance avec Line 4 — gardée comme filet de sécurité rachetable, utile le jour où des starters plus hardcore existeront et qu'un joueur voudra apaiser son jeu sans viser la performance.
- **777/9999** dépendent de la Fusion (valeurs 7 et 9 impossibles autrement que par fusion de deux boutons de base) — signature de fin de run. **Correction (session 30)** : contrairement à ce que disait cette ligne, 777/9999/Skull Line/Shadow Dance/Wedding/Royal Court/Black Hole/Lost Corners/Royal Square/Last Trick/Bottom Corners sont tous **déjà implémentés et jouables** (`ShopManager.SHEET_PATHS`/`LEGENDARY_SHEET_PATHS`, voir la section légendaires ci-dessous) — cette section du doc n'avait jamais été resynchronisée après leur ajout (vagues 1-3). Une passe de resynchronisation complète de ce fichier serait utile (même chantier que celui déjà identifié pour `sortileges-implementes.md`), voir [Questions ouvertes](../meta/questions-ouvertes.md).
- **Rock Bookends** ("SERRE-LIVRES", session 30) — 2 rocks horizontaux + 2-3 jetons scorables entre eux, score = somme des jetons entre, multi = nombre de rocks sur toute la grille au moment du score (`rule`/`shape` = `rock_bookend`). Générique, non légendaire, prix 12.
- **Figures (Valet/Chevalier/Reine/Roi) — mécanisme de promotion revu en profondeur (session 30)** : l'ancienne auto-promotion au score (un jeton à 10+ qui rescorait avançait automatiquement d'un cran) détruisait Wedding/Royal Court dès leur premier succès, puisque ces Sheets exigent une combinaison exacte de figures. Remplacée par un **Couronnement** délibéré, via deux chemins qui coexistent : la Colonne Convoitée (jeton 10+ qui touche un Boost) ou le nouveau Spécial **Couronnement** (Epic, déterministe). Voir [Boutons — Figures](../jetons/boutons.md#figures-arcanes-mineurs), [Colonne Convoitée](../manche/colonne-convoitee.md) et [Spéciaux](../jetons/specials.md).
- **Légendaires (session 30)** : Shadow Dance sort du pool légendaire (`ShopManager.LEGENDARY_SHEET_PATHS`) et rejoint la file uniforme normale — 5 légendaires restent : Lost Corners, Royal Square, Skull Line, Black Hole, Last Trick. Taux de tirage inchangé (`GameRules.LEGENDARY_SHEET_CHANCE` = 3% par slot).
...
Le tiroir rare/signature (9999/Jackpot, Wedding, Royal Court) est en fait déjà implémenté et accessible dans la file générique du shop (voir correction session 30 ci-dessus) — le split d'accès meta décrit ci-dessus (Générique/Verrouillée) ne s'applique donc pas à lui tel quel, une passe de clarification reste à faire (ce troisième niveau était pensé comme non accessible day-one, à re-trancher — voir [Questions ouvertes](../meta/questions-ouvertes.md)).
...
- **Sinon** (chevauchement partiel — typiquement le jeton qui vient d'être posé, point de convergence de deux figures distinctes — **ou aucun chevauchement du tout**, deux figures séparées qui scorent dans la même passe) → **Double Partition** : toutes scorent, total combiné x`GameRules.PATTERN_COMBO_MULTIPLIER` (2 actuellement) par paire de figures accrochées — scale avec le nombre de figures simultanées (N figures dans la même passe paient N x la somme de leurs scores), bannière dédiée. Le cas "aucun chevauchement" (session 23) ne peut arriver que pendant le [Dernier Souffle](../manche/dernier-souffle.md) : un coup normal ne modifie qu'une seule colonne, impossible de compléter deux figures non-adjacentes à la fois — aucun code spécifique au Dernier Souffle n'a été nécessaire, c'est une conséquence mécanique de la règle générale.
