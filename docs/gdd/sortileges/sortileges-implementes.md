# Sortilèges implémentés

**55 Sortilèges actifs** dans le proto (11 batchs — session 27 ajoute Echo et Quitte ou Double, retire Bon départ et Jamais 1 sans 2, tous les deux orphelins de la refonte roulette → [Colonne Convoitée](../manche/colonne-convoitee.md)). Couvrent les 12 triggers (session 13 : `on_token_drop` a eu son premier consommateur, Dernier Carré ; session 16 : `on_round_end` ajouté, premier consommateur Pourboire, voir [Monnaies](../progression/monnaies.md) ; session 17 : `on_level_up` et `on_deck_grown` ajoutés ; session 22 : `on_shake_used`, `on_sheet_sold`, `on_figure_promoted` et `on_deck_tool_shown` ajoutés — voir [Triggers](triggers.md)). Session 23 : 4 premiers Sortilèges Legendary au-delà de Poker Face, dont deux "passifs" (Dresseur Fou, Souffle Obscur) qui ne passent pas par le pipeline de dispatch habituel — même convention qu'Économe, vérifiés directement via `RunManager.has_spell`/`RunContext` là où ils s'appliquent (voir [Rareté des Sortilèges](rarete.md)).

| Sortilège                  | Trigger            | Effet                                                                                                           | Rareté   |
| ---------------------- | ------------------ | --------------------------------------------------------------------------------------------------------------- | -------- |
...
| **Echo**               | *(passif, voir description)* | 15% de chance qu'une Partition qui score se résolve une deuxième fois (retrigger, score doublé) — flag simple sur `RunContext.has_echo`, lu par `CascadeResolver._apply_score_gambles` (session 27) | Epic |
| **Quitte ou Double**   | *(passif, voir description)* | Une Partition qui score joue un pile ou face : 50% double, 50% remet le score de ce groupe à zéro — même mécanisme que Echo, appliqué en premier (session 27) | Epic |

Chaque Sortilège = 1 script d'effet (`scripts/sortileges/effect_*.gd`) + 1 resource (`resources/sortileges/spell_*.tres`). Les 9 Sortilèges "bonus flat au value_sum" et les 3 Sortilèges "scaling permanent" de session 17 partagent chacun une fondation commune : voir [Scoring — bonus flat au value_sum](../partitions/scoring.md#bonus-flat-au-value_sum-session-17) et [Scoring — scaling permanent](../partitions/scoring.md#scaling-permanent-session-17).

## Indicateur de progression au survol (session 13)

`SpellEffect.get_progress_text(run_manager) -> String` — méthode virtuelle générique (chaîne vide par défaut), overridée par les Sortilèges à compteur pour afficher leur état au survol dans `SpellsUI`. Régularité affiche "X/3 tours sans cascade", Un Pour Tous affiche "X/4 familles vues cette manche". Réutilisable pour tout futur Sortilège à compteur, zéro modif de `SpellsUI` nécessaire.
...
Les pistes non implémentées sont brassées dans `brainstorm-sortileges.md` (produit cartésien Triggers × Effets). Seul `on_last_breath` reste câblé sans Sortilège qui le consomme (voir [Triggers](triggers.md)) — `on_token_drop` a bien des consommateurs (Dernier Carré, Jetons sacrés).

## Debug

Chaque Sortilège a un flag `debug_start_equipped: bool` dans son `.tres`. Le cocher dans l'inspector Godot équipe le Sortilège au démarrage de run, bypass shop. Pratique pour playtester un Sortilège isolément.
