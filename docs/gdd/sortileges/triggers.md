...
Les moments où les Sortilèges s'activent. 12 triggers au total — chaque Sortilège en a un.
...
| `on_round_end` | Fin de manche réussie, juste avant l'écran YouWinUI | `SpellManager.dispatch_round_end()` (appelé par `GameScene`, pas un signal `TurnController`) |
| `on_level_up` (session 17) | Une Partition franchit un palier de niveau, en cours de manche | `RunManager.tag_leveled_up` |
| `on_deck_grown` (session 17) | Le pool de boutons vient de grandir (achat au shop, scission) — pas les retraits (fusion, vente) | `RunManager.button_pool_changed`, filtré par delta de taille dans `SpellManager._on_button_pool_changed` |
...
| `on_deck_tool_shown` (session 22) | Un slot "Dé à coudre" vient d'apparaître dans l'offre du shop (au plus 1x par visite, pas à chaque reroll) | `SpellManager.dispatch_deck_tool_shown()` (appelé par `ShopUI._ready()`, pas un signal `TurnController`) |

## Flow de dispatch

`TurnController` émet le signal → `SpellManager._dispatch(trigger, event)` → pour chaque Sortilège équipé dont `sortilège.trigger == trigger`, appel de `sortilège.make_effect().apply(event, run_manager)`.

Le `SpellManager` est câblé à chaque manche via `bind_round(turn_controller)` depuis `GameScene._create_managers()`. Particularité pour `on_level_up`/`on_deck_grown` (session 17) : leurs signaux sources vivent sur `RunManager`, qui persiste toute la run (contrairement à `TurnController`, recréé chaque manche) — `bind_round` s'y connecte quand même par cohérence avec le reste du câblage, mais protégé par `is_connected()` pour ne jamais dupliquer la connexion d'une manche à l'autre.
...
| `on_turn_resolved` | `{timeline, grid_manager}` (grid_manager ajouté session 17, pour les Sortilèges qui mutent la grille après coup — ex: Récif vivant) |
| `on_last_breath` | `{}` (vide) |
| `on_round_end` | `{}` (vide) |
...
11 des 12 ont au moins un Sortilège. `on_round_start` reste le plus chargé (la majorité des Sortilèges à condition statique posée une fois par manche). `on_last_breath` reste câblé mais sans Sortilège qui le consomme.

**Contrainte technique historique, résolue en session 13** : `SpellEffect` est instancié à neuf à chaque dispatch (`sortilège.make_effect()` dans `SpellManager._dispatch`) — un effet ne peut pas accumuler d'état dans ses propres variables entre deux déclenchements. `RunManager` porte un état libre par sortilège (`get_spell_state`/`set_spell_state`, remis à zéro chaque manche) pour les compteurs de manche (streak de Régularité, familles vues de Un Pour Tous), et un second état **jamais remis à zéro en cours de run** (`get_run_spell_state`/`set_run_spell_state`, session 17) pour les Sortilèges "scaling permanent" qui doivent survivre aux transitions de manche (Jetons sacrés, Quatre quart, Escalade musicale, Amélioration continue, Gourmand...).

**Cas particulier "Econome"** : son effet (reroll gratuit au shop) ne passe par aucun trigger — le shop n'a pas de moment "on_shop_enter" dans ce système, câblé uniquement sur les manches. `ShopUI` vérifie directement `RunManager.has_spell(&"econome")` à l'ouverture. Son `trigger` dans `SpellData` n'est donc qu'une valeur de remplissage, jamais réellement dispatché pour cet effet.
