...
| [Sortilèges](../sortileges/principe.md) | Ex : "Mouches en Cascade" → +3 par cascade secondaire. Depuis la session 16, les Sortilèges `on_round_end` (ex : Pourboire) contribuent aussi à la recompense de fin de manche |

### Écran de récompense (YouWinUI, session 16)

En fin de manche (hors dernière manche du run), un écran dédié décompose la récompense — base, bonus jetons restants, bonus par Sortilège — avant un bouton **ENCAISSER** qui débloque la suite vers le shop. Le compteur de mouches affiché en jeu ne bouge visuellement qu'au clic sur ce bouton (la mutation réelle est immédiate, seul l'affichage est retardé, pour ne pas spoiler le total avant que le joueur ait vu le détail).
