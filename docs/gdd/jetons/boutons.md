...
- **Valeur** (chiffre) : 1 à 5 au tirage initial, **plafonnée à 10** par Fusion/Augmenter (`GameRules.MAX_BUTTON_VALUE`, tranché en session 12) — au-delà, voir [Figures](#figures-arcanes-mineurs) ci-dessous, un chemin séparé qui n'existe que via un Couronnement (Colonne Convoitée).
...
**Implémenté session 18, mécanisme de promotion revu session 30.** Un jeton de base déjà à `MAX_BUTTON_VALUE` (10) avance d'un cran dans la suite des figures — Valet(11) → Chevalier(12) → Reine(15) → Roi(20), plafond définitif — via un **Couronnement** : le jeton doit être dropé dans la [Colonne Convoitée](../manche/colonne-convoitee.md) et toucher le palier Boost (n'importe quel tier, Commun/Rare/Légendaire — la magnitude du tier ne change rien pour une figure déjà au plafond, voir `CursedColumnRewards.boosted_value`). Affichage : J/C/Q/K sur le jeton, sprite de famille inchangé. La valeur continue d'alimenter le `value_sum` normalement (11 à 20 points), aucune formule à part.

**Chemin exclusivement accessible par la Colonne Convoitée, jamais par le shop ni par le simple fait de scorer** — Fusion, Augmenter et Poker Face restent plafonnés à 10 (voir `RunManager.increase_button_value`/`fuse_buttons`). **Ancienne version (session 18, abandonnée session 30)** : un jeton à 10+ qui scorait avançait automatiquement d'un cran (`CascadeResolver._roll_face_promotions`, supprimé). Retour user en session 30 : ça détruisait Wedding/Royal Court dès leur premier succès — ces Sheets exigent une combinaison exacte de figures (Roi+Reine pour Wedding, Valet-Chevalier-Reine-Roi alignés pour Royal Court), et la mutation automatique de n'importe quelle figure qui rescore cassait cette combinaison de façon garantie, pas occasionnelle. Nouvelle version : la promotion est un geste délibéré du joueur (il choisit de risquer CE jeton précis dans la Colonne Convoitée, généralement depuis le Hold, avant de le poser), jamais une conséquence automatique du scoring — un Wedding déjà posé sur la grille ne peut donc plus jamais muter (la Colonne Convoitée n'agit que sur le jeton en cours de drop, jamais sur un jeton déjà placé).

**Rareté par construction** : atteindre 10 demande déjà plusieurs Fusions (cher), puis chaque Couronnement est un vrai pari — risque de corruption (le jeton est consommé pour la manche s'il touche le Skull plutôt que le Boost) assumé en connaissance de cause. Une fois promu, un jeton **ne peut plus redescendre** — Réduire est refusé sur une figure (`RunManager.decrease_button_value`). Fusion/Scinder restent ouverts (replafonnent à 10, une façon de "encaisser" une figure dont on ne veut plus).

**Verrouillage ("Fixer", Dés à coudre, rare)** : protège une figure contre tout Couronnement futur — permet de stabiliser volontairement une Reine ou un Chevalier plutôt que de la laisser filer vers Roi. Gratuit hors coût du Dés à coudre lui-même — ce n'est pas un raccourci d'achat vers une figure, juste un contrôle sur un chemin qui reste entièrement gagné par le joueur. Le malus de boss "Cour endormie" bloque tout Couronnement pour la manche, quel que soit le résultat du drop CC (`RunManager.is_figure_promotion_locked`).

**Tension connue** : au-delà de Chevalier, une figure devient orpheline de l'axe casino — Reine(15) et Roi(20) n'ont aucun voisin à distance 1 (pas de Suite possible), et le Brelan demanderait 3 exemplaires de la même figure (structurellement quasi impossible). Seul l'axe famille (Ligne, Carré, Losange, Plus, Cross, Ring, T, Rainbow) reste ouvert passé ce stade. **Wedding** (Roi+Reine adjacents) et **Royal Court** (Valet-Chevalier-Reine-Roi alignés) sont maintenant implémentés comme débouché casino dédié aux figures — voir [Catalogue implémenté](../partitions/catalogue-implemente.md).
